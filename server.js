...
const init = async () => {
    ...
    server.route({
        ...
        handler: {
            directory: {
                path: Path.join(__dirname, 'capstone'), // or 'frontend' or wherever your HTML files live
                listing: false,
                index: true
            }
        }
    ...
    server.route({
        ...
        handler: async (request, h) => {
            ...
            let x =
            {
                ...
                username: 'Daniel',
                password: 'Daniel',
                email: 'daniel.jones.2003@gmail.com',
                street: '123 Fake St.',
                city: 'Fakerville',
                state: 'TX',
                zip: '76087',
                ...
                timestamp: '2025-05-04T22:48:46.320Z',
                createdBy: 0
            };

            const result = await db.collection('Metrics').deleteMany({});
            const a = await db.collection('Templates').deleteMany({});
            const b = await db.collection('Records').deleteMany({});
            const c = await db.collection('MetricTemplates').deleteMany({});
            const d = await db.collection('Users').deleteMany({});
            const e = await db.collection("Users").insertOne(x);
            return d;
        }
    });
    ...
    server.route({
        ...
        handler: async (request, h) => {
          ...
          const filter = { _id: new ObjectId(oldId) };
      
          // Remove any incoming _id from the update payload so we don't try to overwrite it
          const { _id, ...fieldsToSet } = newTemplate;
          ...
          try {
            ...
            return h.response({ success: true, modifiedCount: updateResult.modifiedCount }).code(200);
